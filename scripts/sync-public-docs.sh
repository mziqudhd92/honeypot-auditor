...
for flag in --safe-mode --passive-first --osint-only --dual-stack --profile --jitter --jitter-ms --max-concurrent --seed --preset --format; do
...
for doc in README.md docs/index.html; do
  for needle in --safe-mode --passive-first --osint-only --dual-stack --jitter sarif deception-audit; do
