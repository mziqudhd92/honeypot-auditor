...
def probe_tls_ja4s(host: str, port: int) -> list[Indicator]:
    """Best-effort TLS fingerprint when HTTPS responds."""
    ...
    try:
        ...
        return [
            Indicator(
                id="deep.tls_ja4s",
                title="TLS service fingerprint captured (baseline compare)",
                category="stack_fingerprint",
                triggered=triggered,
                protocol="tls",
                detail=detail,
                evidence=f"handshake={elapsed:.3f}s",
            )
        ]
    except Exception as exc:
        return [
            skipped_indicator(
                "deep.tls_ja4s",
                "TLS service fingerprint captured (baseline compare)",
                "stack_fingerprint",
                closed_reason(str(exc)),
                protocol="tls",
                error=str(exc),
