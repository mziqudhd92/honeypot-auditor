...
def run_deep_probes(host: str, ports: dict[str, int | list[int]], *, corroboration: bool = False) -> list[Indicator]:
    ...
    for ssh_port in ssh_ports:
        ...
        out.extend(_stamp(probe_shell_entropy(host, ssh_port), "ssh", ssh_port))
        out.extend(_stamp(probe_mtime_uniformity(host, ssh_port), "ssh", ssh_port))
        out.extend(_stamp(probe_llm_hallucination(host, ssh_port), "ssh", ssh_port))
    ...
    smb_port = smb_ports[0] if smb_ports else None
    for http_port in http_ports:
        ...
        out.extend(
            _stamp(
                probe_clock_drift(host, http_port, smb_port=smb_port),
                "http",
                http_port,
            )
        )
        if http_port in (80, 8080, 8081):
            out.extend(_stamp(probe_state_continuity(host, http_port), "http", http_port))
    ...
    for smb_port_i in smb_ports:
        out.extend(_stamp(probe_smb_negotiate(host, smb_port_i), "smb", smb_port_i))
        out.extend(_stamp(probe_smb_target_mismatch(host, smb_port_i), "smb", smb_port_i))
